...
parser.add_argument('--model', type=str, default='model/model_value', help='model file save path')
parser.add_argument('--state', type=str, default='model/state_value', help='state file save path')
...
def mini_batch(positions, i, batchsize):
    ...
    return (torch.from_numpy(np.array(mini_batch_data, dtype=np.float32)),
            torch.from_numpy(np.array(mini_batch_win, dtype=np.int64).reshape(-1, 1)))
...
def mini_batch_for_test(positions, batchsize):
    ...
    Returns:
        tuple(2): (テスト用入力特徴量配列, テスト用出力特徴量(指し手)配列)
...
for e in range(args.epoch):
    ...
    for i in range(0, len(positions_train) - args.batchsize, args.batchsize):
        ...
        x, t = mb[0].to(device), mb[1].float().to(device)
        y = model(x).to(device)
        ...
        if current_step % args.eval_interval == 0:
            ...
            x, t = mb[0].to(device), mb[1].to(device)
            y = model(x).to(device)

            logging.info('epoch = {}, iteration = {}, loss = {}, accuracy = {}'.format(
                e + 1, current_step, sum_loss / itr, binary_accuracy(t, y)))
    ...
    with torch.no_grad():
        for i in range(0, len(positions_test) - args.batchsize, args.batchsize):
            ...
            itr_test += 1
            sum_test_accuracy += binary_accuracy(t, y)

        logging.info('epoch = {}, iteration = {}, train loss avr = {}, test accuracy = {}'.format(
            e + 1, current_step, sum_loss_epoch / itr_epoch, sum_test_accuracy / itr_test))
